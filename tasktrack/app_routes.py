...
def app_page(identity):
    return page(
        "Internal apps",
        """<main class="account-content"><a href="/account">Account settings</a><h1>Internal apps</h1><p>Give an internal dashboard read-only access to selected projects. These credentials belong to this workspace and remain active until an administrator revokes them.</p><p id="app-message" role="status"></p><form id="app-form"><label for="app-name">App name</label><input id="app-name" required maxlength="80"><fieldset><legend>Projects this app can read</legend><div id="app-projects"></div></fieldset><label><input type="checkbox" id="app-notes"> Include comments and files</label><p>Your app must verify which signed-in users can see this information. Keep its secret on your server.</p><button class="button primary">Create with passkey</button></form><section id="app-secret" hidden><h2>Save this credential now</h2><p>The secret is shown once. Store it in your app server's secret manager.</p><pre id="app-credential"></pre><button id="app-hide" class="button">I saved it — hide secret</button></section><section><h2>Workspace apps</h2><div id="app-list"></div></section></main>""",
