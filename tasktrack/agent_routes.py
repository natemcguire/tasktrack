...
def agent_page(identity):
    return page(
        "Agent access",
        f"""<main class="account-content setup-page"><a href="/account">Account settings</a>
<nav class="setup-nav" aria-label="Setup"><a href="/agents" aria-current="page">Agents</a><a href="/apps">Internal apps</a><a href="/imports">Imports</a></nav><h1>Agent access</h1><p class="setup-intro">Connect an agent, choose what it can do, and stay in control.</p>
<p>Signed in as <strong>{escape(identity.get("email", ""))}</strong> · <a href="/account">Change account or workspace</a></p>
<p>Workspace: <strong>{escape(identity["workspace_name"])}</strong></p><details class="setup-advanced"><summary>Connection details</summary><p>Workspace ID: <code>{escape(identity["workspace_id"])}</code></p></details>
<section class="account-section setup-primary"><p class="setup-eyebrow">Connect an agent</p><h2>Review a connection request</h2><p>Open the link your agent sent, or enter its code here. You’ll review access before anything is connected.</p><form id="agent-code-form" class="setup-code-form"><label for="agent-code">Code shown by your agent</label><input id="agent-code" maxlength="12" autocomplete="off" autocapitalize="characters" spellcheck="false" required placeholder="ABCD-EFGH"><button class="button primary">Review request</button></form></section>
<p id="agent-message" role="status" aria-live="polite"></p><section id="agent-progress" class="account-section setup-progress" hidden aria-live="polite"><h2 id="agent-progress-title"></h2><p id="agent-progress-detail"></p></section><section id="agent-review" class="account-section setup-primary" hidden><h2>Review access</h2><div id="agent-details"></div><p>Approve only if you initiated this connection. The agent name is supplied by the requesting device.</p><p id="agent-expiry" role="status"></p><button class="button primary" id="agent-approve">Approve with passkey</button> <button class="button" id="agent-deny">Deny</button> <button class="button" id="agent-change">Use another code</button></section>
...
<section class="account-section"><h2>Your agents</h2><div id="agent-list"></div></section><section class="account-section"><h2>Action approvals</h2><div id="approval-list"></div></section>
</main>""",
